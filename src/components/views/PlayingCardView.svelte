...
{#if $gameStore.answerInFocus && !$gameStore.winner && $gameStore.isInRetro && $gameStore.answerInFocus.player}
			...
			{#each $playerStore.answerCards as card}
				<button
					...
					class={classNames(
						...
						{
							'border-4 scale-125 -translate-y-5 shadow-xl': card === selectedCard
						}
					)}
				>
					<h3>{card}</h3>
